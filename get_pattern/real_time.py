...
ADAPTIVE_C = 2            # 適応的二値化の定数 ※小さくすると検出が甘く（線が見えやすく）なる

# === 輪郭近似の精度（途切れ対策） ===
APPROX_EPSILON = 0.02   # 輪郭近似の係数 (0.01～0.03推奨) ※小さいほど細かく検出、大きいほど滑らか

# === とぎれとぎれ対策: 時間的安定化 ===
STABILITY_FRAMES = 1      # 安定化フレーム数（この回数連続で検出されたら表示）
TRACKING_THRESHOLD = 30   # 図形追跡の距離閾値（ピクセル）同一図形と判定する最大距離
